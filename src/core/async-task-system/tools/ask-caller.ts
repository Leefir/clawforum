...
export const askCallerTool: Tool = {
  ...
  async execute(args: Record<string, unknown>, ctx: ExecContext): Promise<ToolResult> {
    ...
    if (!ctx.mainDialogStore || !ctx.mainContextSnapshot) {
      return {
        success: false,
    ...
    try {
      const restored = await ctx.mainDialogStore.restorePrefix(ctx.mainContextSnapshot);
      ...
      if (err instanceof MarkerNotFoundError) {
        return {
          success: false,
          content: `ask_caller: marker not found (toolUseId=${ctx.mainContextSnapshot.toolUseId})`,
