...
dispatch: {
  "task": "探索 clawforum 项目结构，生成报告到 clawspace/clawforum-analysis.md",
  "context": "用户想了解 clawforum 的核心模块",
  "targetClaw": "clawforum-explorer"
}
```

- `targetClaw`：仅当用户明确说"用某某 claw"时填写（含要求新建特定名称的 claw，此时先创建再填入）；否则省略，claw 选择交给 dispatcher 决定
- `task` 和 `context` 无论何时都不含 claw 名称
