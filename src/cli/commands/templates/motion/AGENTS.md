...
dispatcher 会读取 dispatch-skills 简介，按需加载完整模板，通过 `exec` 调 CLI 给指定 claw 创建契约（`clawforum contract create --goal "..." <claw_id>`）。**Dispatcher 不能直接 spawn**——如判断需要 spawn，会在回复中说明建议 prompt，由 Motion 执行。没有匹配模板时自行决策，可保存到 `clawspace/dispatch-skills/` 供下次复用。
