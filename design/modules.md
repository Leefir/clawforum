...
- **依赖**：FileSystem, LLMService, AuditWriter, ToolRegistryImpl(verifier), InboxWriter, execFile/ProcessExec, SkillSystem (L2), TaskSystem (L4 同层)；jobs/ 子目录附加 ProcessExec / Messaging / FS / Audit（contract-observer 周期触发用，与 manager 核心 API 分离）
...
- **被谁调用**：Assembly（主装配 + bind 配置参数 + `cronRunner.start(tickMs)` 启动；jobs 业务逻辑原在 `core/cron/jobs/*.ts`，phase329 起 contract-observer 迁 ContractSystem，剩 disk-monitor / llm-stats / deep-dream / random-dream 待 phase330/331；α.1→α.2 升档进行中，见契约 §7 B.p173-1）
