...
> **应然立场注（phase 444）**：DialogStore 不预设上层模块语义。`clawId` 为 caller 装配选业务关联 / `filename` 与 `archiveDir` 由 caller 注入 / 非模块硬编码。当前签名反映实然（`clawId: string` 必填 / 无 filename 参数）/ 具体接口形态（clawId 改可选或退 schema / archiveDir 注入方式）待 phase 445 code phase derive。
