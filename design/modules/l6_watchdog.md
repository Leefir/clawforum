...
| 跨 agent 目录读 | 派生态 | ✓ 走 L1 fs.listSync（phase 441 closed）|
...
- `watchdog-state.json`：JSON `{ version: 1 (current) | undefined (v0 legacy), lastSeenAt: {...}, motionBackoff: {...}, clawState: {...} }`

### 重建语义

- 进程重启：经 CLI `watchdog start` 重新派生 / loadWatchdogState 容错回退默认（A3 W-tier 静默 catch）
- 状态文件不一致：A2 schema 含 version（v1 current / undefined → v0 legacy 兼容 load）/ closed by phase 493 (git blame watchdog-state.ts:11 确认) / r101 phase 729 同步
...
| ~~A.2~~ `watchdog-state.json` version 字段 | ✅ closed by phase 493 | **closed**（watchdog-state.ts:11 `version?: number; v0 = absent (legacy), v1 = current` + L27 backward compat + L68 write `version: 1`）/ r101 phase 729 design row 同步（src 已 land / row 滞后修正）|
...
| ~~A.6~~ watchdog-entry.ts uncaughtException + unhandledRejection handler | ✅ closed by phase 269 | **closed**（watchdog-entry.ts 18 行 shim 含 process.on('uncaughtException') + process.on('unhandledRejection') 双 handler / 调用 writeWatchdogCrash audit + console.error + exit(1)）/ r101 phase 729 design row 同步 |
| ~~A.7~~ `maybeCronClawCrash` 三分判定 audit | ✅ closed by phase 729 (β has_contract field) | **closed**（watchdog-cron.ts:113 CLAW_SCAN（全表 scan）+ L122 CLAW_CRASH_DETECTED has_contract=false / L130 CLAW_CRASH_DETECTED has_contract=true / L150 CLAW_CRASH_NOTIFY_DROPPED 错误路径 / 三分支全 audit 区分）/ r101 phase 729 β implement |
| ~~A.8~~ 跨 agent fs.readdirSync ≥ 9 处无结构化 audit | ✅ closed by phase 441 | **closed**（watchdog 自己 0 readdirSync / phase 441 改用 L1 fs.listSync({includeDirs:true})）/ "跨 agent readdir" 责任已转 L1 fs.listSync / CLI / assembly 模块 readdir audit 不归本模块 scope reframe / r101 phase 729 design row 同步 |
...
- M#4 持久化：watchdog-state.json 跨进程持久化（A.2 closed / A.3-A.4 W-tier 治理候选）
...
- D1c 中断可恢复：watchdog-state.json 跨重启共享（A.2 closed by phase 493）
...
- D10 多 claw 不隔绝：跨 agent 目录读经 L1 fs.listSync（phase 441 closed）/ 监督本质需跨 claw
...
- **跨 agent 目录读**：经 L1 fs.listSync({includeDirs:true}) / phase 441 closed by （watchdog 自己 0 readdirSync）/ 单 agent 失败不阻塞其他（L1 内部 catch）
- **maybeCronClawCrash 三分判定**：cron scan（CLAW_SCAN）+ 真崩溃有 contract（CLAW_CRASH_DETECTED has_contract=true）+ 真崩溃无 contract（CLAW_CRASH_DETECTED has_contract=false）/ 三分支全 audit / A.7 closed by phase 729 β
