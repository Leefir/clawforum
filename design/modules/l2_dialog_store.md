...
- **`<dialogDir>/<filename>`**：caller 注入文件名（如主 claw 'current.json' / subagent 'messages.json'）/ 模块本身不预设。
- **`<archiveDir>/<ts>_<uuid8>.json`**：caller 装配 archive 目录（如主 claw `<dialogDir>/archive/`）。
...
| **A.r61-1 DialogStore 实然预设上层模块语义（4 点）** | drift / 中 → high | open / 推 r+ code phase（phase 445+ 实施 / 应然修订 closed by phase 444 design 重审）| **应然已 sharpen**：filename + clawId + archiveDir 路径 + SessionData.clawId 4 预设点违 D5 底层不预设上层 + M#1。**实然 drift**：(1) ctor 内 `path.join(dialogDir, 'current.json')` / (2) ctor 第 4 参 `clawId: string` 必填 + `SessionData.clawId` 必字段 / (3) ctor 内 `path.join(dialogDir, 'archive')` / (4) `save()` 自动包 `SessionData` 含 `clawId` 必填。**真合规设计**：filename caller 注入 / clawId 退化为可选业务关联（subagent 用例 0 clawId）/ archiveDir caller 装配（缺省可保 'archive' subdir）/ SessionData.clawId 字段可选。源：r61 phase 444 design 重审 / path D / 用户拍板「DialogStore 不预设上层模块语义」|
...
- **M3 资源归属**：`<dialogDir>/<filename>` + `<archiveDir>/` 归 DialogStore 独占
...
- **M9 显式表达编译器可检**：LoadResult.source 强类型 union / DialogData interface / `clawId` 字段 caller 装配选（非模块强制必填 / 具体可选或退 schema 待 phase 445 定）
...
### 7.F Design-Gap 登记（应然 silent / 待用户补 / 0 mechanical 决定）

> per `feedback_design_gap_when_yingran_silent`：应然层未识别的业务概念归属 / 主会话不预决 / 待用户拍板。

| Design-Gap | 类型 | 状态 | 说明 |
|---|---|---|---|
| **G1 主对话 'current.json' 文件名概念 own** | design-gap | 应然 silent / 待用户补 | 主 claw daemon 装配 DialogStore 时选 `filename='current.json'` / 是 caller 业务约定 / 「主对话 current 文件」概念归 claw daemon 业务。需登记 `modules/<claw daemon>.md`（若新立模块）或 `modules.md` / 应然层未明 |
| **G2 clawId 业务关联 own** | design-gap | 应然 silent / 待用户补 | 主 claw daemon 装配时填 clawId 业务关联 / DialogStore `SessionData.clawId` 仅是 passthrough metadata / 实际「dialog ↔ claw 关联」业务概念 own 在 claw daemon。subagent 用例 0 clawId（无 claw 关联 / 是 task 内 ephemeral run）|
| **G3 archive rotation 业务策略 own** | design-gap | 应然 silent / 待用户补 | `archive()` 触发已是 caller / `archiveDir` 配置归 caller 装配 / 「主对话 archive 时机」业务概念 own 在 claw daemon（如轮次切换 / 上下文压缩等）|
| **G4 dialog session 跨进程 lifecycle 概念 own** | design-gap | 应然 silent / 待用户补 | 「主对话 session」业务概念（含 clawId + archive + version 演化）跨进程 lifecycle 管理 / 现 implicit 散在 DialogStore + claw daemon / 应然层未识别该业务模块 / 是不是新模块 own「主对话 session 管理」？还是归 claw daemon？待用户补 design |
