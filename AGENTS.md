...
### 2.7.8 新增 CLI 命令必须执行验证（新增，Step 31.4 教训）

**问题**: Step 30 新增的 `claw list` 命令有 Bug（使用了 NodeFileSystem 不存在的 sync 方法），但 218 个测试全部通过。直到 Step 31.4 实际执行命令时才被发现。

**根本原因**: 
- `--help` 只能验证命令注册，不能验证功能实现
- TypeScript 编译通过不代表运行时方法存在
- `catch` 块吞掉了 `TypeError`，变成模糊错误信息

**规则**: **每个新增的 CLI 命令必须至少执行一次并检查输出**
...
```bash
# ❌ 不充分：只验证命令存在
node dist/cli.js claw --help  # 通过，但 list 功能实际不可用

# ✅ 正确：验证实际功能
node dist/cli.js claw list    # 验证能列出 claws
node dist/cli.js claw status test-claw  # 验证能读取状态
```
