...
### 模式 1: 类型字段凭记忆编写 🔴
**描述**: 实现时不查看类型定义，凭记忆写字段名/类型值
**发生次数**: 3+ 次（monitor/, react/, communication/）
**影响**: 每次需要 2-3 轮修复
**解决方案**: 写实现前必须先 ReadFile 类型定义
**状态**: 待观察（已加入 CONVENTIONS.md 检查清单）

### 模式 2: 路径层级混淆 🟡
**描述**: 混用 IFileSystem 相对路径和原生 fs 绝对路径
**发生次数**: 2 次（fs/ 测试, communication/ 测试）
**影响**: 测试中文件找不到
**解决方案**: 统一使用 IFileSystem 抽象，路径都是相对于 baseDir
**状态**: 已缓解（fs.list 修复后更清晰）
