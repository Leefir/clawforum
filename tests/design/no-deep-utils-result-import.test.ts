...
describe('phase 1432 F7: utils/result barrel-only invariant', () => {
  it('cross-module deep imports `from "*/utils/result.js"` baseline ratchet = 0 (excluding SDK)', () => {
    ...
    try {
      hits = execSync(
        `grep -rnE "from ['\\\"][^'\\\"]*utils/result\\.js['\\\"]" ${srcRoot} --include='*.ts' | grep -vE "^${srcRoot}/foundation/utils/" | grep -v "^${srcRoot}/index.ts:"`,
        { encoding: 'utf8' },
      );
    } catch (e: any) {
  ...
  it('反向自检 — path-prefix anchor 只排除 owner module 内部、不误排除 cross-module deep import (phase 1440 治 P0-2 substring false-green)', () => {
    ...
    const ownerInternal = `${srcRoot}/foundation/utils/index.ts:5:export { ok } from './result.js';`;
    const crossModuleDeep = `${srcRoot}/core/runtime/runtime.ts:99:import { ok } from '../../foundation/utils/result.js';`;
