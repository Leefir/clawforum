 ...
 *
 * Targets:
 * - 'crash_notification' 字面（Watchdog L6 业主）→ ML#5 反向防护
 * - 'HEARTBEAT.md' 路径（Heartbeat L5 业主）→ ML#2/#3 业务归属防护
 * - 'HEARTBEAT_AUDIT_EVENTS' import（Heartbeat L5 业主）→ ML#3 audit 归属防护
 *
 * Allowed:
...
describe('phase 1414 invariant: Runtime 不字面持上下游 inbox 业务', () => {
  it("0 hit for 'crash_notification' literal（Watchdog 业主、ML#5 反向防护）", () => {
  ...
  it("0 hit for 'HEARTBEAT.md' path（Heartbeat 业主 FS 读、ML#2/#3 防护）", () => {
    const blob = readAllTs(RUNTIME_DIR);
    expect(blob.includes("'HEARTBEAT.md'")).toBe(false);
    expect(blob.includes('"HEARTBEAT.md"')).toBe(false);
  });

  it('0 hit for HEARTBEAT_AUDIT_EVENTS const import / use（Heartbeat 业主 audit、ML#3 防护）', () => {
    const blob = readAllTs(RUNTIME_DIR);
