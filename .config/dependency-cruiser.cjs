...
module.exports = {
  forbidden: [
    ...
    {
      name: 'no-deep-into-pm-factories',
      comment: [
        'ML#7 + ML#9 — createProcessManagerForCLI 跨模块暴露通道仅 barrel。',
        '跨模块 caller（cli/, watchdog/）只能 import',
        'src/foundation/process-manager/index.ts、不得深穿',
        'src/foundation/process-manager/factories.ts。',
        'phase 1416 F1 立、treat finding `A.phase1416-pm-factories-deep-import` ⏳ → ✅。',
        'scope: 本规则仅治 factories.ts。sister deep imports（agent-factory.ts /',
        'paths.ts / signal-clean-stop.ts）按 case-by-case 评估、不一律 barrel-only。',
        '示例 fix: import { createProcessManagerForCLI } from "../../foundation/process-manager/index.js"',
        '而非 "../../foundation/process-manager/factories.js"',
      ].join(' '),
      severity: 'error',
      from: { path: '^src', pathNot: '^src/foundation/process-manager/' },
      to: { path: '^src/foundation/process-manager/factories\\.ts$' },
    },
    {
